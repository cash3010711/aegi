  ...
  <div class="main_content">
    <div class="row-fluid">
      <div class="span12 project_detail">
        <h2><a href="{{url('/dashboard')}}">總覽</a> / <a href="{{url('/dashboard/admin')}}">系統管理員</a> / 刪除 - 還原 {{$type}}</h2>
        <!-- Button -->
        @if($data != null)
        <div class="restore_sec">
          <a data-toggle="modal" href="#myModal-restoreall" class="restore_btn">全部還原</a>
          <a data-toggle="modal" href="#myModal-deleteall" class="restore_del_btn">全部永久刪除</a>
        </div>
        <!-- Project -->
            ...
            <div class="view_proj_inner">
              <div class="view_proj_title">
                ...
                <div class="view_proj_btn">
                  <a href="#" class="restore_btn2 restorethis" entityid={{$entity['id']}} type={{strtolower($type)}}>還原</a>
                  <a href="#" class="restore_del_btn2 deletethis" entityid={{$entity['id']}} type={{strtolower($type)}}>永久刪除</a>
                </div>
              </div>
              <h3>{{$entity['name']}}</h3>
              @if($type == 'Events')
              <span class="eventdate">日期: {{new ExpressiveDate($entity['date'])}}</span>
              @endif
              <div class="projectview_list_sec">
                <div class="projectview_list">
                  <div class="projectview_left">建立日期:</div>
                  <div class="projectview_right">
                    <p>{{new ExpressiveDate($entity['created_at'])}}</p>
                  </div>
                </div>
                <div class="projectview_list">
                  <div class="projectview_left">更新日期:</div>
                  <div class="projectview_right">
                    <p>{{new ExpressiveDate($entity['updated_at'])}}</p>
                  </div>
                </div>
                <div class="projectview_list">
                  <div class="projectview_left">刪除日期:</div>
                  <div class="projectview_right">
                    <p>{{new ExpressiveDate($entity['deleted_at'])}}</p>
                  </div>
                </div>
                <div class="projectview_list">
                  <div class="projectview_left">刪除者:</div>
                  <div class="projectview_right">
...
<div id="myModal-deleteall" class="modal cal_light_box hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
    <div class="confirm-delete">確定刪除全部{{$type}}嗎?</div>
    <div class="confirm-button">
    <form method="post" action="{{url('/dashboard/admin/data/deleteall',array(strtolower($type)))}}">   <button class="submit">確定</a></button></form>
  <button class="submit dontdelete" id="dontdelete" >取消</a></button></div>
</div>
</div>
...
<div id="myModal-restoreall" class="modal cal_light_box hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
    <div class="confirm-delete">確定還原全部{{$type}}嗎?</div>
    <div class="confirm-button">
    <form method="post" action="{{url('/dashboard/admin/data/restoreall',array(strtolower($type)))}}">   <button class="submit">確定</a></button></form>
  <button class="submit dontdelete" id="dontdelete" >取消</a></button></div>
</div>
</div>
...
<div id="entity-delete" class="modal cal_light_box hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
    <div class="confirm-delete">確定刪除?</div>
    <div class="confirm-button">
      <form method="post" action="{{url('/dashboard/admin/data/delete')}}">  <input type="hidden" name="entitytype" id="entitytype" value=""  > <input type="hidden" name="entityid" id="entityid" value=""  ><button class="submit">確定</a></button></form>
    <button class="submit dontdelete" id="dontdelete" >取消</a></button></div>
  </div>
</div>
<!-- end delete popup -->
...
<div id="entity-restore" class="modal cal_light_box hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
    <div class="confirm-delete">確定還原?</div>
    <div class="confirm-button">
      <form method="post" action="{{url('/dashboard/admin/data/restore')}}">  <input type="hidden" name="restoreentitytype" id="restoreentitytype" value=""  > <input type="hidden" name="restoreentityid" id="restoreentityid" value=""  ><button class="submit">確定</a></button></form>
    <button class="submit dontdelete" id="dontdelete" >取消</a></button></div>
  </div>
</div>
<!-- End restore popup -->
